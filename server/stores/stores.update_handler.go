...
func (api *StoresAPI) Update(ctx *fiber.Ctx) error {
	input := &do.UpdateStoreReq{}
	if err := ctx.BodyParser(input); err != nil {
		err := v_proto.NewRpcError(int32(v_proto.VolioRpcErrorCodes_BAD_REQUEST), err.Error())
		v_log.V(1).WithError(err).Errorf("StoreAPI::Update - Error: %+v", err)
	...
	if err := api.storesController.Update(input, models.KTypeStoreData); err != nil {
		v_log.V(1).WithError(err).Errorf("StoreAPI::Update - Error: %+v", err)

		return v_api.WriteError(ctx, err)
	}

	v_log.V(3).Infof("StoreAPI::Update - Reply: Oke")

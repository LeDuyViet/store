...
func (api *StoresAPI) Gets(ctx *fiber.Ctx) error {
	storeID := ctx.Query("id", "")
	if storeID != "" {
		res := api.storesController.Get(storeID, models.KTypeStoreData)
	...
	res := api.storesController.Gets(offset, limit, models.KTypeStoreData)

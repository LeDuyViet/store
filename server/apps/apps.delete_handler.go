/* !!
 * File: apps.delete_handler.go
 * File Created: Wednesday, 26th July 2023 2:41:51 pm
 * Author: KimEricko™ (phamkim.pr@gmail.com)
 * -----
 * Last Modified: Wednesday, 26th July 2023 3:01:13 pm
...
func (api *AppsAPI) Delete(ctx *fiber.Ctx) error {
	id := ctx.Params("id")
	if id == "" {
		err := v_proto.NewRpcError(int32(v_proto.VolioRpcErrorCodes_BAD_REQUEST), "store id is required")
		v_log.V(1).WithError(err).Errorf("AppsAPI::Delete - Error: %+v", err)

		return v_api.WriteError(ctx, err)
	}

	if err := api.appsController.Delete(id, models.KTypeAppData); err != nil {
		v_log.V(1).WithError(err).Errorf("AppsAPI::Delete - Error: %+v", err)

		return v_api.WriteError(ctx, err)
	}

	v_log.V(3).Infof("AppsAPI::Delete - Reply: Oke")

...
func (api *CategoriesAPI) Create(ctx *fiber.Ctx) error {
	input := &do.CreateCategoryReq{}

	if err := ctx.BodyParser(input); err != nil {
		err := v_proto.NewRpcError(int32(v_proto.VolioRpcErrorCodes_BAD_REQUEST), err.Error())
		v_log.V(1).WithError(err).Errorf("CategoriesAPI::Create - Error: %+v", err)
	...
	err := api.categoriesController.Create(input, models.KTypeCategory)
	if err != nil {
		v_log.V(1).WithError(err).Errorf("CategoriesAPI::Create - Error: %+v", "can not create store")

		return v_api.WriteError(ctx, err)
	}

	v_log.V(3).Infof("CategoriesAPI::Create - Reply: %s", base.JSONDebugDataString(input))
